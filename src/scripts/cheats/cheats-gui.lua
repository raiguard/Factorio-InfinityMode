...
local function create_cheat_ui(parent, obj, cheat, player_is_god, player_is_editor)
    ...
    elseif cheat_def.type == 'number' then
        ...
        element = setting_flow.add{type='textfield', name='im_cheats-'..cheat_name..'-textfield', style='short_number_textfield', text=tostring(cheat_def.functions.get_value(obj, util.cheat_table(cheat[1], cheat[2], obj.index)) or '---'), numeric=true, lose_focus_on_confirm=true}
    end
...
local function create_tabbed_pane(player, window_frame)
    ...
    for group,cheats in pairs(elems_def.player.toggles) do
        ...
        for i,n in pairs(cheats) do
            create_cheat_ui(flow, player, {'player',n}, player_is_god, player_is_editor)
        end
    end
    ...
    local forces_list = upper_flow.add{type='list-box', name='im_cheats_force_listbox', style='list_box_in_tabbed_pane', items=forces, selected_index=player_table.cheats_gui.cur_force.index}
